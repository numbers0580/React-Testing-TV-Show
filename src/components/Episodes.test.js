...
test('Displays episodes', () => {
    render(<Episodes episodes={[]} />);
});

test('Display episodes from API call', () => {
    const epsList = render(<Episodes episodes={[]} />);

    epsList.rerender(<Episodes episodes={EPISODE_LIST.data}/>); //passed array of episodes above as props

    let episodeData = EPISODE_LIST.data;
    episodeData.forEach(episode => {
        expect(epsList.getByText(episode.name)).toBeInTheDocument();
    });
});