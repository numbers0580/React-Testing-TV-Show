...
//test('', () => {}); //Empty test setup. Saving so I could copy-paste it as needed

test('App renders correctly when mounting.', () => {
    mockFetchShow.mockResolvedValueOnce(STRANGER_THINGS_DATA);
...
test('Testing the "loading" Message', () => {
    ...
    expect(msg.getByText(/fetching/i)).toBeInTheDocument();
});

test('Testing data from API', async () => {
    mockFetchShow.mockResolvedValueOnce(STRANGER_THINGS_DATA);
    //const {rerender, findByText} = render(<App />);
    //rerender(<App />);

    const {findByText, findAllByRole} = render(<App/>);
    const dropmenu = await findByText(/select/i);
    userEvent.click(dropmenu); //menulist threw an error without this userEvent line
    const menulist = await findAllByRole(/option/i);
    expect(dropmenu).toBeInTheDocument();
    expect(menulist).toHaveLength(4);
    expect(menulist[0]).toBeVisible(); //Tested to see if "Season 1" was showing after userEvent

    menulist.forEach(season => {
        expect(season).toBeVisible(); //Tested to see if "Season 1" ... "Season 4" were all showing after userEvent
    });
});